...
@pytest.mark.parametrize(
    ...
    [
        (
            UserUpdate(
                first_name="Jan", last_name="Kowalski", email="john@example.com"
            ),
            User(
                id=uuid.uuid4(),
                first_name="Jan",
...
async def test_update_user_exclude_unset(mock_session, db_user, user_update):
    expected_first_name = (
        user_update.first_name if user_update.first_name else db_user.first_name
    )
    expected_last_name = (
        user_update.last_name if user_update.last_name else db_user.last_name
    )
    result = await update_user(mock_session, db_user, user_update)
    mock_session.flush.assert_called_once()
