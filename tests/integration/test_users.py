...
async def test_update_user(client_httpx, create_user_payload, update_user_payload):
    first_name = (
        update_user_payload["first_name"]
        if update_user_payload.get("first_name") is not None
        else create_user_payload["first_name"]
    )
    last_name = (
        update_user_payload["last_name"]
        if update_user_payload.get("last_name") is not None
        else create_user_payload["last_name"]
    )
...
@pytest.mark.integration
async def test_update_user_not_found(client_httpx, payload):
    response = await client_httpx.patch("/v1/users", json=payload)
...
async def test_update_user_invalid_payload(
    client_httpx, create_user_payload, update_user_payload
):
    response = await client_httpx.post("/v1/users", json=create_user_payload)
    assert response.status_code == 201
    response = await client_httpx.patch("/v1/users", json=update_user_payload)
