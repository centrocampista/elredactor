...
@pytest.mark.parametrize(
    "expected_response", [(ExpextedResponse(status_code=200, body={"status": "ok"}))]
)
@pytest.mark.integration
async def test_health_positive(client_fastapi, test_settings, expected_response):
    ...
    response = client_fastapi.get("/health")
