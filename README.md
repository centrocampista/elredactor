...
Document ingestion and retrieval service built with FastAPI. Uploaded documents are stored in
PostgreSQL for structured queries and (in progress) indexed into a Qdrant vector store for
semantic search. An LLM agent layer built on LangGraph runs as a separate service.

> **Status: work in progress.** Upload, users and API-credential auth work end to end.
> Chunking, embedding and retrieval are not wired up yet — see [Roadmap](#roadmap).
...
- **Vector store** — Qdrant, managed through the application lifespan
- **API key authentication** — `X-Api-Key` / `X-Api-Secret` credential pairs issued per user
- **Agent service** — LangGraph graph exposed separately on port `2024`, model access via
  OpenRouter, optional LangSmith tracing
...
```
   ...
   client ──▶ nginx ──▶  elredactor  │  FastAPI  :8000
          ...
          ▼                   ▼                  ▼
   ┌─────────────┐     ┌────────────┐     ┌─────────────┐
   │ postgres_red│     │   qdrant   │     │  langgraph  │
   │  metadata   │     │  vectors   │     │   agents    │
   │    :5432    │     │ :6333/6334 │     │    :2024    │
   └─────────────┘     └────────────┘     └─────────────┘
```

Uploaded files land on a shared volume (`/content`), the metadata row is written to PostgreSQL,
and the document is queued for indexing.
...
- [ ] Background worker to move documents through `processing` → `done`
- [ ] Text extraction (PyMuPDF, docx2txt) and chunking
- [ ] Embedding and indexing into Qdrant
- [ ] Semantic search / retrieval endpoint
- [ ] RAG agent wired to the indexed corpus
